...
ACCEPTED_PARAMS = [
    ...
    pytest.param("L=1.0 K3=1.0 DISFRIN=1", id = "disfrin"),
]

@pytest.mark.parametrize("params", ACCEPTED_PARAMS)
def test_oct_accepts(sad_accepts, params):
    """
    SAD's OCT element should accept K3, the standard
...
def test_oct_rejects(sad_rejects, params):
    ...
    (K0-K2/SK0-SK4), solenoid (BZ), RF parameters, and FRINGE/F1/F2/FB1/
    FB2/F1K1x/F2K1x -- OCT has no FRMD/soft-edge-fringe keyword at all
    (unlike BEND/QUAD/MULT), only DISFRIN.
    """
    sad_rejects(
        f"OCT O1 = ({params});\n"
        "MARK START = ()\n     END   = ();\n"
...
################################################################################
def _track_oct_probe(tmp_path, k3, disfrin_suffix, name):
    """
    Track a single off-axis particle through a K3 OCT and return the
    track_sad result.
    """
    lat = tmp_path / name
    lat.write_text(
        "MOMENTUM = 1.0 GEV;\n"
        f"OCT O1 = (L=1.0 K3={k3}{disfrin_suffix});\n"
        "MARK START = ()\n     END   = ();\n"
        "LINE TEST = (START O1 END);\n")
    cwd = os.getcwd()
    os.chdir(tmp_path)
...
def test_oct_disfrin_default_matches_explicit_zero(tmp_path):
    """
    DISFRIN unset defaults to DISFRIN=0 (hard-edge fringe enabled) --
    bit-identical output, not just approximately equal.
    """
    r_unset = _track_oct_probe(tmp_path, 0.4, "", "oct_disfrin_unset.sad")
    r_zero  = _track_oct_probe(tmp_path, 0.4, " DISFRIN=0", "oct_disfrin_zero.sad")
...
def test_oct_disfrin_is_boolean(tmp_path):
    """
    DISFRIN is a strict boolean gate on an OCT, same as on BEND/QUAD/SEXT:
    any nonzero value disables the hard-edge fringe identically.
    """
    disfrin_1 = _track_oct_probe(tmp_path, 0.4, " DISFRIN=1", "oct_disfrin_bool_1.sad")["px"][0]
    for disfrin in (2, -1, 3, 0.5):
        r = _track_oct_probe(tmp_path, 0.4, f" DISFRIN={disfrin}", f"oct_disfrin_bool_{disfrin}.sad")
...
def test_oct_disfrin_hard_edge_matches_sad_reference_values(tmp_path):
    """
    Pinned ground truth: L=1.0, K3=0.4, x=2E-2/px=2E-2/y=1.5E-2/py=-1E-2 on
    entry, with the hard-edge fringe enabled (DISFRIN=0/unset) and
    disabled (DISFRIN=1). Real SAD binary outputs, recorded once and
    locked in.
    """
    r_on  = _track_oct_probe(tmp_path, 0.4, "", "oct_disfrin_ref_on.sad")
    r_off = _track_oct_probe(tmp_path, 0.4, " DISFRIN=1", "oct_disfrin_ref_off.sad")

    assert r_on["px"][0] == pytest.approx(0.019998583655016534, rel=1e-6), (
        "SAD's px with the hard-edge fringe enabled no longer matches the "
        "pinned reference value.")
    assert r_off["px"][0] == pytest.approx(0.019998582704700753, rel=1e-6), (
