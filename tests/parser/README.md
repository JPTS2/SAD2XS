...
| `test_lines.py` | 6 | 0 | — |
| `test_line_names.py` | 3 | 3 | Parser strips the token `line` from element/line names that contain it — all three tests fail |
...
### `test_line_names.py` note

All three tests fail. The parser incorrectly strips the token `line` from any
name that contains it (e.g. `MYLINE` becomes `MY`). This is a known parser
limitation that breaks lattices where element or line names contain the word
`line`.
