...
- `test_supported_elements.py` — supported-element policy: one test
  ...
  tested as distinct writer paths.
...
- `test_value_formatting.py` — `get_value_string`, the single place every
  ...
  infinity and NaN written as `float(...)` calls, expression strings safely
  escaped as double-quoted Python literals, and `get_knl_string` sharing the
  same formatting.
