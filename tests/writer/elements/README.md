...
- `test_refshift_writer.py` — `xt.Translation` (shift_x/shift_y), `xt.TimeDelay` (shift_zeta), `xt.YRotation` (chi1), `xt.XRotation` (chi2), `xt.SRotation` (chi3); all as optics expressions; zero default_to_zero behaviour; all five types in one line
- `test_aper_writer.py` — `xt.LimitEllipse` (a/b), `xt.LimitRect` (min/max x/y), offsets, asymmetric bounds, mixed types; optics variable tests expected to FAIL (issue #62 — dimensions not yet written as live expressions). Note: tracking-grid tests are not needed here; field-value equality is sufficient because the writer serialises Python values to Python code with no interpretation step — physical correctness is Xsuite's contract, not SAD2XS's.
