...
- `test_taylor_maps_writer.py` — `xt.FirstOrderTaylorMap` (m0/m1) and `xt.SecondOrderTaylorMap` (k/R/T) as literal arrays (not optics variables), full double precision, the `_sad_quad_fringe_a/b/theta` reversal-metadata attributes `_new_quad_fringe_element` stashes on a quad-fringe map (preserved when present, absent when not), minus-sign root-name cleanup on both map orders (stripped without a non-minus sibling, kept when both exist), multiple maps in one line
