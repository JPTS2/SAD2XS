...
The two sets do not map one to one. A single SAD element can become several Xsuite elements: a bound solenoid becomes a four-component sub-line, an RF-carrying `MULT` becomes alternating multipole and cavity slices, and a quadrupole with fringe import enabled becomes up to three elements.
