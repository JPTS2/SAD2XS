...
from ._000_helpers import extract_multipole_information, get_value_string, \
    ...
    check_is_skew_quad_sext_oct, get_knl_string, get_variablename
...
########################################"""
    ...
    for quad, quad_length in zip(quad_names, quad_lengths):
        for replica_name in quads[quad_length]:
            ...
            variable_name = get_variablename(source_name)
            ...
            if check_is_simple_quad_sext_oct(line, source_name, "Quadrupole"):

                if not check_is_skew_quad_sext_oct(line, source_name, "Quadrupole"):
                    output_string += f"""
env.new(name = "{replica_name}", prototype = "{quad}", k1 = "k1_{variable_name}")"""
                else:
                    output_string += f"""
env.new(name = "{replica_name}", prototype = "{quad}", k1s = "k1s_{variable_name}")"""

            else:
                # Get the replica information
                k1          = line[source_name].k1
                k1s         = line[source_name].k1s
                shift_x     = line[source_name].shift_x
                shift_y     = line[source_name].shift_y
                rot_s_rad   = line[source_name].rot_s_rad
                knl         = np.asarray(line[source_name].knl)
                ksl         = np.asarray(line[source_name].ksl)
...
env.new(
    ...
    prototype   = "{quad}\""""
                ...
                if k1 != 0:
                    quad_generation += f""",
    k1          = "k1_{variable_name}\""""
                if k1s != 0:
                    quad_generation += f""",
    k1s         = "k1s_{variable_name}\""""
...
def create_quadrupole_optics_file_information(
    ...
    unique_quad_variables = list(dict.fromkeys(
        get_variablename(name) for name in unique_quad_names))
    for quad in unique_quad_variables:
        k1          = None
        k1s         = None
